...
import { executeAssociateCall, suggestAssociate } from '@/lib/associateExecutor';
...
export async function POST(
  ...
  try {
        ...
        aiAssociate: {
          select: {
            ...
            steps: {
              ...
              select: { id: true, description: true, stepOrder: true },
            },
            tools: {
    ...
    // ── Associate-bound detection ─────────────────────────────────────────────
    // When the conversation is bound to a specific associate via aiAssociateId,
    // that associate becomes the sole primary responder for every message in this
    // conversation. The orchestrator (wansom) is bypassed; system tools are still
    // available to the associate via invoke_orchestrator_tool.
    const boundAssociate = conversation?.aiAssociate ?? null;
    const isAssociateBound = !!boundAssociate;
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // ── Associate-bound execution path ───────────────────────────────────────
          // When the conversation has a bound associate, skip the orchestrator
          // system prompt and Gemini stream entirely. The associate runs its own
          // bounded agentic loop, with invoke_orchestrator_tool as the escape hatch
          // for any system-level tool it needs (canvas editing, document generation, etc.).
          if (isAssociateBound) {
            // messageHistory is DESC (most-recent first). Take 5 most recent, reverse to chronological order,
            // and carry role so the associate can distinguish user turns from previous assistant turns.
            const recentMsgs = messageHistory
              .slice(0, 5)
              .reverse()
              .map((msg: any) => ({ role: msg.role as string, content: msg.content as string }));
            const associateTempId = `temp-${Date.now()}`;
            ...
            const associateResult = await executeAssociateCall(
              { name: '', args: { query: content, context: relevantContent } },
              projectId,
              project,
              conversationDocuments,
              recentMsgs,
            ...
            const formattedAssociateContent = formatAIMessage(responseText);

            // Extract document metadata if a document was generated by the associate
            const associateDocumentMetadata = associateResult.documentMetadata ?? null;

            // Emit the complete response as a single delta so the client renders it
            controller.enqueue(
              encoder.encode(
                JSON.stringify({
                  type: 'delta',
                  conversationId: conversation.id,
                  messageId: associateTempId,
                  content: formattedAssociateContent,
                }) + '\n'
              )
            );

            // Update conversation timestamp + persist message + run passive suggestion check — all in parallel.
            // The associate's own tool-triggered suggestion (suggestedAssociateFromTool) takes priority
            // over the passive check so we skip the passive check when the associate already signalled one.
            const toolSuggestion = associateResult.suggestedAssociate ?? null;
            const [assistantMessage, passiveSuggestion] = await Promise.all([
              (async () => {
                await prisma.conversation.update({
                  where: { id: conversationId },
                  data: { updatedAt: new Date() },
                }).catch(console.error);
                // Build metadata to persist (document card + any future fields)
                const msgMetadata: any = {};
                if (associateDocumentMetadata?.document) {
                  msgMetadata.document = {
                    title: associateDocumentMetadata.document.title,
                    format: associateDocumentMetadata.document.format,
                    htmlContent: associateDocumentMetadata.document.htmlContent,
                  };
                }
                const metadataToSave = Object.keys(msgMetadata).length > 0
              ...
              // Only run the passive check when the associate didn't already surface one
              toolSuggestion ? Promise.resolve(null) : suggestAssociate(content, boundAssociate.id, projectId),
            ]);

            const finalSuggestion = toolSuggestion ?? passiveSuggestion;

            // Emit final event (with optional associate suggestion and document card)
            controller.enqueue(
              encoder.encode(
                JSON.stringify({
                  type: 'final',
            ...
            // Signal completion and return — orchestrator path is skipped
            controller.enqueue(
              encoder.encode(
                JSON.stringify({
                  type: 'status',
