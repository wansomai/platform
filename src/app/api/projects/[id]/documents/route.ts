...
export async function GET(
  ...
  try {
    ...
    const formattedDocuments = projectDocuments.map((pd) => ({
      ...
      contentExtracted: Boolean(pd.document.content_extracted), // True if content extraction status is valid and true
...
export async function POST(
  ...
  try {
    ...
    console.error('Error attaching documents to project:', error);
    return NextResponse.json(
      { message: 'Failed to attach documents to project', error: true },
      { status: 500 }
    );
  }
}
