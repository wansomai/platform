...
export function ChatSidebar() {
  ...
  const { fetchChatHistory, createConversation, conversationId } = useChat(projectId)
  ...
  const fetchConversations = async () => {
    try {
      setIsLoading(true)
      const endpoint = apiEndpoints.conversations.list(projectId)
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-3 border-b border-secondary-200 flex justify-between items-center">
        <h2 className="font-medium text-secondary-900">Conversations</h2>
        <Button 
            ...
            {filteredConversations.map((conversation) => (
              <div
                ...
                className={`group flex items-start p-4 hover:bg-secondary-50 cursor-pointer ${
                  conversationId === conversation.id ? 'bg-secondary-50' : ''
                }`}
              >
