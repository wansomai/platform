...
/** Generate a 12-char alphanumeric temporary password (no ambiguous chars). */
...
export default function ActivationModal({ open, onOpenChange }: ActivationModalProps) {
  ...
  // Stored after auth — used in step 2 to activate
  ...
  const getJurisdictionNames = () =>
    jurisdictions.map((code) => COUNTRIES.find((c) => c.value === code)?.title || code);

  const getTopicLabels = () =>
    topics.map((t) => (PRACTICE_AREA_LABELS as Record<string, string>)[t] || t);
  ...
  /** Authenticate (login or auto-register), store access token, advance to step 2. */
  const handleStep1Next = async () => {
    ...
    try {
      ...
      if (isExistingUser) {
        ...
        // Auto-register — no extra fields needed
      ...
      <DialogContent className="sm:max-w-[560px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl">
            {step === 1 ? 'Set Up Your Legal Digest' : 'Review & Activate'}
          </DialogTitle>
        ...
        {step === 1 && (
          <div className="space-y-5 py-2">
            ...
            <div>
              ...
              <div className="flex gap-2 mt-1.5">
                <Input
                  ...
                  disabled={isCheckingEmail || isAuthenticating}
                />
                {!emailChecked && (
                  <Button
            ...
            <Button
              ...
              disabled={isAuthenticating || isCheckingEmail || !emailChecked}
            >
              {isAuthenticating ? (
                <>
        ...
        {step === 2 && (
          <div className="space-y-5 py-2">
            ...
            {!isExistingUser && (
              <div className="flex items-start gap-2 p-2.5 bg-amber-50 border border-amber-100 rounded-lg">
                ...
                <p className="text-xs text-amber-700">
                  A welcome email with your login details will be sent to{' '}
                  <strong>{email}</strong> after activation.
                </p>
              </div>
            )}
            ...
            <div className="flex items-center justify-center gap-1.5 text-xs text-gray-400">
              ...
              <span>Secure payment · No spam · Cancel anytime</span>
            </div>
          </div>
        )}
      </DialogContent>
