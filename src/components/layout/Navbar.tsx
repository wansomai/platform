  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 ${
      ...
      <div className="section-container mx-auto px-4 py-4 flex justify-between items-center">
        ...
        {/* Mobile menu button */}
        <button 
          className={`lg:hidden z-50  focus:outline-none transition-transform duration-300 ${darkmode ? 'text-black' : 'text-primary'}`}
          onClick={() => setIsOpen(!isOpen)}  aria-label="Mobile menu"
        >
          {isOpen ? <X className={darkmode?'text-primary':'text-black'} size={24} /> : <Menu size={24} />}
        </button>
